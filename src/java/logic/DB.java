...
//                LogErrorManager.getInstance().addError(3, "DB.query(String " + q + ")", "FATAL: Query not complite ("+e.toString()+")");
                //warning is here
                rset.close();
                stmt.close();
                return true;
    ...
    public void printRes() {
        if (iserror) {
            LogErrorManager.getInstance().addError(2, "DB.printRes()", "can't print result");
            return;
        }
