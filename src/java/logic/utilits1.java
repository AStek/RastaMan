...
public class utilits1 {
    ...
    public static void getSeparator(JspWriter out, int width){
        try {
            ...
            Logger.getLogger(utilits1.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public static void getMenuItem(JspWriter out, String title, int width, String id, String info){
        try {
            String rTitle = null;
    ...
    public static void getTimelineMenu(JspWriter out, ArrayList<HashMap> list){
        try{
            Boolean first = true;
            String title  = null;
            Date   dat    = new Date();
            ...
            if (!list.isEmpty()){
                ...
                out.print(title+" : <div class=\"timeline\">");
            }
...
//
//                    getMenuItem(out, name, getPercentDateInterval(startTime,
//                                endTime), item.get("ID").toString(), info);
...
//
//                    getMenuItem(out, name, width, item.get("ID").toString(), info);
//                    dat = getDateFromString(item.get("END_TIME").toString());
                }
                    if (!first){
                       getSeparator(out, getPercentDateInterval(
